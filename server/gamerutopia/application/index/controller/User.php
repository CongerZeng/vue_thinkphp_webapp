...
define('KEY', 'dwedwedwqeqweqwdqwddqwd22hyyy');
...
class User extends Controller
{
	...
	public function register() {
		...
		$pwd = input('pwd');
		$time = input('time');
		$ava = (string)(rand(100,200));
		$avatar = 'https://image.gamersky.com/avatar/original/game/game'.$ava.'.jpg';
		 $result = model('User')->register($email,$name,$pwd,$avatar,$time);
		 if (!$result[0]) {
    ...
    public function getUserInfo() {
		$name = input('name');
		$userInfo = model('User')->getUserInfo($name);
		$postInfo = model('User')->getPostInfo($name);
		$result = ['userInfo'=>$userInfo,'postInfo'=>$postInfo];
        if($result) {
            return $result;
        }
	}
